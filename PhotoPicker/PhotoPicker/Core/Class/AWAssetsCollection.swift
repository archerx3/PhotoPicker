...
public struct AWPHAsset {
    ...
    //Bool to check if AWPHAsset returned is created using camera.
    ...
    public var fullResolutionImage: UIImage? {
        get {
            ...
            return AWPhotoLibrary.fullResolutionImageData(asset: phAsset)
        }
    }
    ...
    public func cloudImageDownload(progressBlock: @escaping (Double) -> Void, completionBlock:@escaping (UIImage?)-> Void ) -> PHImageRequestID? {
        guard let phAsset = self.phAsset else { return nil }
        return AWPhotoLibrary.cloudImageDownload(asset: phAsset, progressBlock: progressBlock, completionBlock: completionBlock)
    }
    ...
    public static func asset(with localIdentifier: String) -> AWPHAsset? {
        let fetchResult = PHAsset.fetchAssets(withLocalIdentifiers: [localIdentifier], options: nil)
        return AWPHAsset(asset: fetchResult.firstObject)
    }
}

extension AWPHAsset: Equatable {
    public static func ==(lhs: AWPHAsset, rhs: AWPHAsset) -> Bool {
...
public struct AWAssetsCollection {
    ...
    public var sections: [(title: String, assets: [AWPHAsset])]? = nil
    var count: Int {
        get {
    ...
    func getAWAsset(at indexPath: IndexPath) -> AWPHAsset? {
        ...
        if let sections = self.sections {
            ...
            guard let result = self.fetchResult, index < result.count else { return nil }
            return AWPHAsset(asset: result.object(at: max(index,0)))
        }
    }
    ...
    mutating func reloadSection(groupedBy: PHFetchedResultGroupedBy) {
        ...
        if self.useCameraButton {
            groupedSections.insert(("camera",[AWPHAsset(asset: nil)]), at: 0)
        }
        self.sections = groupedSections
    }
    
    static func ==(lhs: AWAssetsCollection, rhs: AWAssetsCollection) -> Bool {
